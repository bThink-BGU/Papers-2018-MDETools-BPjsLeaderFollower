...
def getTelem():
    ...
    roverSocket.sendall(b"player1,GPS()\n")
    ...
    roverSocket.sendall(b"player1,getCompass()\n")
    rData = repr(roverSocket.recv(1024))
...
def logger():
       while True:
              ...
              if  LL.as_long() == RR.as_long():
                  stringout="player1,moveForward("+str(LL)+")\n"
              ...
              stringout="player1,setSuction("+str(Suction)+")\n"
              roverSocket.sendall(stringout.encode('utf-8'))
              print (stringout)
...
roverSocket.connect(('127.0.0.1', 9001))
getTelem()
time.sleep(0.1)
run([
       bounds(),
